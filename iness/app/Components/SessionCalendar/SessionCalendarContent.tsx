...
export interface SessionCalendarContentProps {
  /** Session list from API (active only is filtered inside) */
  sessionData: any[] | null | undefined;
  loading: boolean;
  error?: string | null;
  /** When set, open directly to this day's sessions (YYYY-MM-DD) */
...
/**
 * Modular session calendar content: calendar grid + session list for selected day.
 * Use inside a bottom sheet or modal. When initialDate is set, shows that day's sessions immediately.
 */
function SessionCalendarContent({
  sessionData,
  loading,
  error,
  initialDate,
...
}: SessionCalendarContentProps) {
  const theme = useGlobalTheme();
  const { isDark } = useTheme();
  const styles = getStyles(theme, isDark);
  const [selectedDate, setSelectedDate] = useState<string | null>(null);

  // Open directly to initial day when provided (e.g. from session card "View")
  useEffect(() => {
    if (initialDate) {
        ...
        <View style={styles.headerRow}>
          <View style={styles.headerLeft}>
            ...
            <Text style={styles.sessionType}>
              {session.sessionType?.toUpperCase?.() ?? "SESSION"} Session
            </Text>
          </View>
          <View
            style={{
          ...
          >
            <Text
              style={[
                ...
                {
                  color: session.color || theme.colors.secondPrimary,
                },
              ]}
            >
    ...
    <View style={styles.container}>
      ...
      {!loading && selectedDate && selectedSessions && (
        <ScrollView showsVerticalScrollIndicator={false}>
          {selectedSessions.map((session: any) => renderSessionCard(session))}
        </ScrollView>
      )}
    </View>
  );
...
export default SessionCalendarContent;
